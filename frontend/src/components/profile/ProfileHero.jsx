import { CalendarDays, Eye, Heart, Pencil, Sparkles } from 'lucide-react'
import { Link } from 'react-router-dom'
import { formatProfileDate } from './profileUtils'

export default function ProfileHero({ editable = true, isCreator = false, onOpenCreatorProfile, profile = {}, publicProfileTo = '' }) {
  ...
  return (
    <section className="profile-hero">
      ...
      {editable ? <div className="profile-hero__actions"><Link className="profile-button profile-hero__edit" to="/settings#profile"><Pencil aria-hidden="true" size={15} /> Edit Profile</Link>{publicProfileTo ? <Link className="profile-button profile-button--soft" to={publicProfileTo}><Eye aria-hidden="true" size={15} /> Public preview</Link> : null}{isCreator && onOpenCreatorProfile ? <Link className="profile-button profile-button--soft" onClick={onOpenCreatorProfile} to="/creator/profile"><Sparkles aria-hidden="true" size={15} /> Creator profile</Link> : null}</div> : null}
