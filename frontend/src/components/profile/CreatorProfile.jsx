...
const CREATOR_PROFILE_FALLBACK = Object.freeze({
  about:
    'This creator contributes original music that celebrates Singapore\'s National Day, shared identity, and community stories.',
  bio:
    'Turning Singapore\'s stories, sounds, and memories into music for everyone to explore.',
  creatorSince: '2025',
  displayName: 'Creator',
  ...
  return [...songs]
    ...
    .sort((first, second) => {
      const statusDifference =
        (STATUS_ORDER[first.status] ?? 9) -
        (STATUS_ORDER[second.status] ?? 9)
...
function CreatorProfileHero({ onOpenUserProfile, profile, publicProfileTo }) {
  ...
  return (
    <section className="creator-profile-hero">
      <div
        aria-label={`${profile.displayName} creator avatar`}
        className="creator-profile-avatar"
      >
        {profile.avatarUrl ? (
          <img
            alt={`${profile.displayName} creator portrait`}
      ...
      <div className="creator-profile-hero__copy">
        <p className="creator-profile-eyebrow">
        ...
        <div className="creator-profile-name">
          ...
          <span
            ...
            className="creator-profile-verified"
            role="img"
          >
        ...
        <div className="creator-profile-metadata">
          ...
          <span>
            ...
            Creator since {profile.creatorSince}
          </span>
        </div>

        <div
          aria-label="Creative focus"
          className="creator-profile-tags"
        >
      ...
      <svg
        aria-hidden="true"
        className="creator-profile-skyline"
        viewBox="0 0 520 190"
      >
...
function CreatorProfileStats({ counts, loading }) {
  const items = [
    {
      detail: 'Live in the public library',
      icon: Music2,
      label: 'Published songs',
      value: counts.PUBLISHED || 0,
    },
    {
      detail: 'Drafting or generating',
      icon: FilePenLine,
      label: 'In-progress songs',
      value:
        (counts.DRAFT || 0) +
        (counts.GENERATING || 0),
    },
    {
      detail: 'Prepared for release',
      icon: CheckCircle2,
      label: 'Ready to publish',
      value: counts.READY || 0,
    },
    {
      detail: 'Across every workflow stage',
      icon: Disc3,
      label: 'Total songs',
      ...
      {items.map(
        ({
          ...
          <article key={label}>
            ...
            <div>
              {loading ? (
                <span className="creator-profile-skeleton creator-profile-skeleton--number" />
              ) : (
                <strong>{value}</strong>
              )}
...
function RhythmAvailability({ beatmaps }) {
  ...
  return (
    <span
      className={`creator-profile-rhythm ${
        published ? 'is-ready' : ''
      }`}
    >
...
function CreatorSongCard({ beatmaps, song }) {
  ...
  const date = published
    ? song.publishedDate || song.updatedAt
    : song.updatedAt
  ...
  return (
    <article className="creator-profile-song-card">
      <div className="creator-profile-song-card__art">
        ...
        <span
          className={`creator-profile-status is-${
...
function CreatorSongCollection({
  beatmapsBySong,
  error,
  loading,
  onRetry,
    ...
    <section className="creator-profile-section creator-profile-collection">
      ...
      {loading ? (
        <div className="creator-profile-song-grid">
          {[1, 2, 3].map((value) => (
            <span
              className="creator-profile-skeleton creator-profile-skeleton--song"
              key={value}
            />
          ))}
        </div>
      ...
      !songs.length ? (
        <div className="creator-profile-empty">
          ...
          <h3>
            The collection is waiting for
            its first song
          </h3>

          <p>
            {profile.displayName}
            &apos;s published work and
            active drafts will appear
            here.
          </p>
  ...
  return (
    <section className="creator-profile-about">
      <div className="creator-profile-about__copy">
        <p className="creator-profile-section-kicker">
      ...
      <Music2
        aria-hidden="true"
        className="creator-profile-about__music"
      />
    </section>
  )
}
    ...
    <section className="creator-profile-section creator-profile-community">
      ...
      {loading ? (
        <div className="creator-profile-community-grid">
          {[1, 2].map((value) => (
            <span
              className="creator-profile-skeleton creator-profile-skeleton--reflection"
              key={value}
            />
          ))}
        </div>
      ...
      !reflections.length ? (
        <div className="creator-profile-empty creator-profile-empty--compact">
          ...
          <h3>
            Stories will gather here
          </h3>

          <p>
            As listeners begin sharing
            memories inspired by{' '}
            {profile.displayName}
            &apos;s songs, selected
...
function CreatorFeaturedQuote({
  profile,
}) {
  return (
    <aside
      aria-label="Featured artist quote"
      className="creator-profile-quote"
    >
      ...
      <span
        aria-hidden="true"
        className="creator-profile-quote__waveform"
      />
    </aside>
  )
}
...
const activityPresentation = {
  ...
  FAILED: {
    copy: 'Generation needs attention',
    icon: Clock3,
  },
  PROCESSING: {
    copy: 'Generation in progress',
    icon: WandSparkles,
  },
  QUEUED: {
...
function CreatorActivity({
  ...
  return (
    <section className="creator-profile-section creator-profile-activity">
      <ProfileSectionHeader
        subtitle="Recent work behind the music"
      ...
      {loading ? (
        <span className="creator-profile-skeleton creator-profile-skeleton--activity" />
      ) : null}
          ...
          {uniqueJobs.map((job) => {
            ...
            return (
              <li key={job.id}>
                <span
                  aria-hidden="true"
                  className={`creator-profile-timeline__icon is-${job.status?.toLowerCase()}`}
                >
                  <Icon />
                </span>
                ...
                <span
                  className={`creator-profile-timeline__status is-${job.status?.toLowerCase()}`}
                >
                  {job.status || 'UPDATED'}
                </span>
              </li>
      ...
      !uniqueJobs.length ? (
        <div className="creator-profile-empty creator-profile-empty--compact">
          ...
          <h3>
            No recent studio activity
          </h3>

          <p>
            New generation work will be
            recorded here.
          </p>
        </div>
      ) : null}
...
function SectionError({
  ...
  return (
    <div
      className="creator-profile-error"
      role="alert"
    >
      <p>{message}</p>
...
export default function CreatorProfile() {
  ...
  const profile = useMemo(
    () => ({
      ...
      about: savedProfile
        ? savedProfile.bio
        : CREATOR_PROFILE_FALLBACK.about,
      avatarUrl:
        savedProfile?.avatarUrl || '',
      bio: savedProfile
        ? savedProfile.tagline
        : CREATOR_PROFILE_FALLBACK.bio,
      displayName:
        savedProfile?.displayName ||
        user?.name ||
        CREATOR_PROFILE_FALLBACK.displayName,
      languages: Array.isArray(
        savedProfile?.languages
      )
        ? savedProfile.languages
        : CREATOR_PROFILE_FALLBACK.languages,
      location: savedProfile
        ? savedProfile.location
        : CREATOR_PROFILE_FALLBACK.location,
      quote: savedProfile
        ? savedProfile.featuredQuote
        : CREATOR_PROFILE_FALLBACK.quote,
      role: savedProfile
        ? savedProfile.creatorTitle
        : CREATOR_PROFILE_FALLBACK.role,
      creatorSince:
        savedProfile?.creatorSince ||
        (user?.createdAt &&
        !Number.isNaN(
          new Date(
            user.createdAt
      ...
      themes: Array.isArray(
        savedProfile?.contentFocus
      )
        ? savedProfile.contentFocus
        : CREATOR_PROFILE_FALLBACK.themes,
    }),
    [savedProfile, user]
  )
  ...
  const load = useCallback(
    async () => {
      ...
      if (
        ...
        const beatmapAuthenticationFailed =
          beatmapResults.some(
            (result) =>
              result.status ===
                'rejected' &&
        ...
        setBeatmapsBySong(
          Object.fromEntries(
            visibleSongs.flatMap(
              (song, index) =>
                beatmapResults[index]
                  .status ===
                'fulfilled'
                  ? [
                      [
      ...
      if (
        ...
        nextErrors.activity =
          summaryResult.reason
            ?.message ||
          'Unable to load recent studio activity.'
      }
      ...
      if (
        ...
        nextErrors.community =
          reflectionResult.reason
            ?.message ||
          'Unable to load community reflections.'
      }
  ...
  const orderedSongs = useMemo(
    () => sortCollection(songs),
    [songs]
  )

  const counts = useMemo(() => {
    const byStatus = songs.reduce(
      (current, song) => ({
        ...current,
        [song.status]:
          (current[song.status] ||
  ...
  return (
    <div className="creator-profile">
      {location.state
        ?.profileSaved ? (
        <div
          className="creator-profile-success"
          role="status"
        >
          Your public profile was
