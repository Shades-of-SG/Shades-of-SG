...
export default function ResetPassword({ afterCompletePath = '/login', onComplete, requestPath = '/forgot-password' }) {
  ...
  // An administrator-issued reset link (Admin Users → Reset Password) lands here with
  // ?token=... already carrying an authorized reset session, skipping the email/OTP steps
  // entirely and landing straight on the "choose a new password" form below.
  const [searchParams] = useSearchParams(); const linkToken = searchParams.get('token') || ''
  const [code, setCode] = useState(''); const [resetToken, setResetToken] = useState(linkToken); const [password, setPassword] = useState(''); const [confirmPassword, setConfirmPassword] = useState(''); const [error, setError] = useState(''); const [fieldErrors, setFieldErrors] = useState({}); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false)
  ...
  async function complete(event) {
    ...
    setBusy(true)
    try { await completePasswordReset(resetToken, password); sessionStorage.removeItem('passwordResetEmail'); setMessage('Password updated successfully. You can now sign in.'); setResetToken('COMPLETE'); onComplete?.() } catch { setError(linkToken ? 'This reset link is invalid or has expired. Ask an administrator to send a new one.' : 'The reset session is invalid or expired. Request a new code.') } finally { setBusy(false) }
  }
  ...
  if (!email && !resetToken) return <div className="auth-form"><h1>Reset session missing</h1><Link to={requestPath}>Request a reset code</Link></div>
