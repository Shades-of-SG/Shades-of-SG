...
export default function Register() {
  ...
  useEffect(() => {
    ...
    setFieldErrors((current) => ({ ...current, confirmPassword: debouncedConfirmPassword === form.password ? '' : 'Passwords do not match.' }))
  }, [debouncedConfirmPassword, form.password])
  ...
  return <form className="auth-form auth-form--register" noValidate onSubmit={handleSubmit}>
    ...
    <label className="field-stack"><span>Password</span><span className="field-hint" id="register-password-requirements">{PASSWORD_REQUIREMENTS_TEXT}</span><span className="password-field"><input aria-describedby={`register-password-requirements${fieldErrors.password ? ' register-password-error' : ''}`} aria-invalid={Boolean(fieldErrors.password)} aria-label="Password" autoComplete="new-password" maxLength="128" minLength="8" onChange={(event) => update('password', event.target.value)} placeholder="Create a password" required type={showPassword ? 'text' : 'password'} value={form.password} /><PasswordToggle isVisible={showPassword} onToggle={() => setShowPassword((current) => !current)} /></span>{fieldErrors.password ? <span className="field-hint field-hint--error" id="register-password-error">{fieldErrors.password}</span> : null}</label>
