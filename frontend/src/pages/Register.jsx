...
function validateRegistration(values) {
  ...
  const passwordIssueList = passwordIssues(values.password)
  ...
  if (passwordIssueList.length) {
    errors.password = passwordIssueList.join(' ')
  }
...
export default function Register() {
  ...
  const debouncedEmail = useDebouncedValue(form.email.trim(), 400)
  const debouncedPassword = useDebouncedValue(form.password, 400)
  const debouncedConfirmPassword = useDebouncedValue(form.confirmPassword, 400)
  useEffect(() => {
  ...
  const confirmPasswordError = fieldErrors.confirmPassword
    || (debouncedConfirmPassword && debouncedConfirmPassword !== form.password ? 'Passwords do not match.' : '')
  ...
  return <form className="auth-form auth-form--register" noValidate onSubmit={handleSubmit}>
    ...
    <label className="field-stack"><span>Email</span><input aria-describedby={emailError ? 'register-email-error' : undefined} aria-invalid={Boolean(emailError)} autoComplete="email" onChange={(event) => update('email', event.target.value)} placeholder="Enter your email" required type="email" value={form.email} />{emailError ? <span className="field-hint field-hint--error" id="register-email-error">{emailError}</span> : null}</label>
    <label className="field-stack"><span>Password</span><span className="field-hint" id="register-password-requirements">{PASSWORD_REQUIREMENTS_TEXT}</span><span className="password-field"><input aria-describedby={`register-password-requirements${passwordError ? ' register-password-error' : ''}`} aria-invalid={Boolean(passwordError)} aria-label="Password" autoComplete="new-password" maxLength="128" minLength="8" onChange={(event) => update('password', event.target.value)} placeholder="Create a password" required type={showPassword ? 'text' : 'password'} value={form.password} /><PasswordToggle isVisible={showPassword} onToggle={() => setShowPassword((current) => !current)} /></span>{passwordError ? <span className="field-hint field-hint--error" id="register-password-error">{passwordError}</span> : null}</label>
    <label className="field-stack"><span>Confirm password</span><span className="password-field"><input aria-describedby={confirmPasswordError ? 'register-confirm-password-error' : undefined} aria-invalid={Boolean(confirmPasswordError)} aria-label="Confirm password" autoComplete="new-password" maxLength="128" minLength="8" onChange={(event) => update('confirmPassword', event.target.value)} placeholder="Confirm your password" required type={showConfirmPassword ? 'text' : 'password'} value={form.confirmPassword} /><PasswordToggle isVisible={showConfirmPassword} label="password confirmation" onToggle={() => setShowConfirmPassword((current) => !current)} /></span>{confirmPasswordError ? <span className="field-hint field-hint--error" id="register-confirm-password-error">{confirmPasswordError}</span> : null}</label>
