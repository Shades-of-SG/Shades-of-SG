      ...
      {selected ? <SectionCard className="creator-song-detail">
        ...
        <div className="creator-song-actions">
          <div className="creator-song-actions__icons">
            ...
            <button aria-label={selected.status === 'ARCHIVED' ? 'Unarchive song' : 'Archive song'} className="creator-song-icon-button" disabled={busyId === selected.id || (selected.status === 'GENERATING' && hasActiveGeneration)} onClick={() => toggleArchive(selected)} title={selected.status === 'ARCHIVED' ? 'Unarchive song' : 'Archive song'} type="button">{selected.status === 'ARCHIVED' ? <ArchiveRestore aria-hidden="true" /> : <Archive aria-hidden="true" />}</button>
            <button aria-label="Delete song" className="creator-song-icon-button is-danger" disabled={busyId === selected.id || (selected.status === 'GENERATING' && hasActiveGeneration)} onClick={() => confirmDelete(selected)} title="Delete song" type="button"><Trash2 aria-hidden="true" /></button>
          </div>
          {latestStatus === 'COMPLETED' && selected.latestGenerationJob?.id
            ? <button className="creator-song-generate-button" disabled={busyId === selected.id} onClick={() => navigate(`/creator/editor/${selected.latestGenerationJob.id}`)} type="button"><Video aria-hidden="true" />Open Video Editor</button>
