...
const upload = multer({
  ...
  fileFilter(req, file, callback) {
    ...
    const error = new Error(
      'Invalid file type. Use MP3, WAV, M4A, WebM, or MP4.',
    )
...
const coverUpload = multer({
  ...
  fileFilter(req, file, callback) {
    ...
    const error = new Error(
      'Invalid cover image type. Use JPG, PNG, or WebP.',
    )
    error.statusCode = 400
    return callback(error, false)
  },
...
router.get(
  ...
  requireCreator,
  songController.getCreatorDashboardSummary,
)

router.get(
  '/creator',
  requireCreator,
  songController.listCreatorSongs,
)

router.get(
  '/creator/:id',
  requireCreator,
  songController.getCreatorSong,
)

router.post(
  '/extract-audio',
  requireCreator,
  songController.extractAudio,
)
...
router.post(
  ...
  songController.uploadSongVideo,
)
...
router.post(
  ...
  songController.uploadCoverImage,
)

router.get(
  '/:id/readiness',
  requireCreator,
  songController.getPublishReadiness,
)
