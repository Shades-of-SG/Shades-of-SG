...
function buildSongValues(body, { partial = false } = {}) {
  ...
  // Transitional aliases used by existing frontend clients.
  if (
    body.lyrics !== undefined
    && body.rawLyrics === undefined
  ) {
    values.rawLyrics = body.lyrics
  }

  if (
    body.youtubeUrl !== undefined
    && body.sourceYoutubeUrl === undefined
  ) {
    values.sourceYoutubeUrl = body.youtubeUrl
  }
  ...
  if (
    ...
    values.languages = [body.language]
  }
  ...
  for (const field of [
    ...
    if (values[field] !== undefined) {
      values[field] = normalizeArray(values[field])
    }
  ...
  if (values.transcriptionSegments !== undefined) {
    const normalized = normalizeJson(
      values.transcriptionSegments,
      'transcriptionSegments',
    )
    ...
    if (
      ...
      return {
        error:
          'transcriptionSegments must be an array or null.',
      }
    }

    values.transcriptionSegments = normalized.value
  }
  ...
  if (values.title !== undefined) {
    ...
    if (values.title.length > 255) {
      return {
        error:
          'Song title must be 255 characters or fewer.',
      }
    }
  }

  if (
    values.durationSecs !== undefined
    && values.durationSecs !== null
  ) {
    ...
    if (
      ...
      return {
        error:
          'durationSecs must be a non-negative integer.',
      }
    }
...
function publishValidation(song) {
  ...
  if (
    !Array.isArray(song.languages)
    || song.languages.length === 0
  ) {
    missing.push('languages')
  }

  if (!song.rawLyrics?.trim()) {
    missing.push('rawLyrics')
  }

  if (!song.coverImageUrl?.trim()) {
  ...
  if (
    !['READY', 'PUBLISHED'].includes(song.status)
  ) {
    missing.push('status READY')
  }

  return missing
}

async function reconcileCompletedGeneration(
  song,
  latestJob,
) {
  if (
    ...
    && song.videoUrl
    && ['DRAFT', 'GENERATING'].includes(song.status)
  ) {
    await song.update({
      status: 'READY',
    })
...
async function listPublicSongs(req, res, next) {
  try {
    ...
    const filtered = songs.filter((song) => {
      ...
      const languages = (song.languages || []).map(
        (value) => String(value).toLowerCase(),
      )

      const moods = (song.moodTags || []).map(
        (value) => String(value).toLowerCase(),
      )
...
async function listCreatorSongs(req, res, next) {
  try {
    ...
    if (req.query.status) {
      ...
      if (!SONG_STATUSES.has(status)) {
        return res.status(400).json({
          message: 'Invalid song status.',
        })
      }
    ...
    return res.json({
      songs: songs.map(serializeCreatorSong),
    })
  } catch (error) {
    return next(error)
  }
...
function serializeCreatorSong(song) {
  const value = song.get({
    plain: true,
  })

  const jobs = [
    ...(value.generationJobs || []),
  ].sort(
    (a, b) =>
...
async function getCreatorDashboardSummary(
  req,
  res,
  next,
) {
  try {
    ...
    return res.json({
      ...
      recentSongs:
        recentSongs.map(serializeCreatorSong),
...
async function createSong(req, res, next) {
  try {
    ...
    if (req.file) {
      ...
      parsed.values.sourceYoutubeUrl
      && !audioUrl
    ) {
      const extracted =
        await audioExtractionService
  ...
  try {
    ...
    if (song.status === 'GENERATING') {
      return res.status(409).json({
        message:
          'A generating song cannot be edited.',
      })
    }

    const parsed = buildSongValues(
      req.body,
      {
        partial: true,
      },
...
async function publishSong(req, res, next) {
  try {
    ...
    await reconcileCompletedGeneration(
      song,
      latestJob,
    )
  ...
  try {
    ...
    if (
      ...
      await cloudinaryService
        ...
        .catch((error) => {
          console.error(
            `Unable to delete replaced cover ${previousPublicId}:`,
            error.message,
          )
        })
    }
...
async function uploadSongVideo(
  req,
  res,
  next,
) {
  try {
    ...
    if (song.status === 'GENERATING') {
      return res.status(409).json({
        message:
          'Please wait for the current video generation to finish before uploading another video.',
      })
    }
    ...
    const uploaded =
      await aiStorageService.uploadVideoStream(
        req.file.buffer,
      )
    ...
    if (
      ...
      await cloudinaryService
        ...
        .catch((error) => {
          console.error(
            `Unable to delete replaced video ${previousPublicId}:`,
            error.message,
          )
        })
    }
...
async function unpublishSong(req, res, next) {
  try {
    ...
    if (song.status !== 'PUBLISHED') {
      return res.status(409).json({
        message:
          'Only a published song can be unpublished.',
      })
    }
...
async function archiveSong(req, res, next) {
  try {
    ...
    if (song.status === 'GENERATING') {
      return res.status(409).json({
        message:
          'A generating song cannot be archived.',
      })
    }
...
async function deleteSong(req, res, next) {
  try {
    ...
    if (song.status === 'GENERATING') {
      return res.status(409).json({
        message:
          'A generating song cannot be deleted.',
      })
    }
...
async function extractAudio(req, res, next) {
  try {
    ...
    const extractedInfo =
      await audioExtractionService
        .extractAudioFromYouTube(youtubeUrl)

    try {
      const fileStream =
        fs.createReadStream(
          extractedInfo.filePath,
        )

      const audioData =
        await aiStorageService
          .uploadAudioStream(fileStream)
